...
sqs.createQueue(params, function(err, data) {
 if (err) console.log(err, err.stack); // an error occurred
  ...
  waitingSQS = false;
  createMessages_v3(data.QueueUrl);
 }
});
...
async function createMessages_v1(queueUrl) {
 var messages = [];
 for (var a = 0; a < 50; a++) {
...
//batch method with <=256 kb payload, using sendMessageBatch, cost saver, creates 50 msgs, 5 calls to sqs instead of 50

async function createMessages_v2(queueUrl) {
 var messages = [];
 for (var a = 0; a < 5; a++) {
  messages[a] = [];
  for (var b = 0; b < 10; b++) {
...
function createMessages_v3(){
 var message = 'This is a message from Amazon SNS';
 console.log('Sending messages: '+ message);
 sns.publish({
 Message: message,
  TargetArn: 'arn:aws:sns:us-east-2:175028240505:backspace-sqssnsnodejs-lab' }, function(err,
data) {
 if (err) {
